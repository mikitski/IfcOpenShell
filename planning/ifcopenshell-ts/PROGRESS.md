...
(both parts). **`util.alignment` landed** (PR #64) — see the Phase 4 table below for detail. **Next
Phase 4 dispatch**: `util.shape` is the one remaining Phase 4 Tier B module — but note this module
operates almost entirely on `W.triangulation`/kernel-generated geometry (`ifcopenshell.geom` output),
a much bigger blocker than its 3 `shapely`/`unary_union` call sites alone (`polygon-clipping` is
confirmed installable, the npm registry is reachable in this sandbox even though `pip`/PyPI is not,
but that alone doesn't unblock most of the module) — worth re-scoping/re-confirming with a careful
read of the real source before dispatching, rather than assuming the roadmap's "just needs
polygon-clipping" framing covers the whole module. Phase 5 (`selector.py`) is fully complete. Phase 4
(`util` Tier B) now has `placement`/`geolocation`/`representation`/`cost`/`shape_builder` (both
parts)/`alignment` landed.
