...
(both parts). **Next Phase 4 dispatch**: `util.alignment` is in flight — a small module (111 lines,
4 functions), but 3 of its 4 functions call into the entirely-unported `ifcopenshell.api.alignment`
(a Phase 6+ concern, not a Phase 4 one) and are genuinely, completely blocked; only `station_as_string`
is portable now. After it lands, the one remaining Phase 4 Tier B module is `util.shape` — but note
this module operates almost entirely on `W.triangulation`/kernel-generated geometry (`ifcopenshell.geom`
output), a much bigger blocker than its 3 `shapely`/`unary_union` call sites alone (`polygon-clipping`
is confirmed installable, the npm registry is reachable in this sandbox even though `pip`/PyPI is not,
but that alone doesn't unblock most of the module) — worth re-scoping/re-confirming with a careful read
of the real source before dispatching, rather than assuming the roadmap's "just needs polygon-clipping"
framing covers the whole module. Phase 5 (`selector.py`) is fully complete. Phase 4 (`util` Tier B) now
has `placement`/`geolocation`/`representation`/`cost`/`shape_builder` (both parts) landed.
