...
Phase 3 Tier A item) and/or the remaining Phase 4 Tier B modules — `util.shape_builder` (currently
in flight, split into two chunks given its unusual size: 2312 lines/60 methods; Part 1 dispatched
covering the vector-math free functions + `ShapeBuilder` class minus the four MEP transition/bend
methods, Part 2 to follow for those), `util.shape` (needs `polygon-clipping`, confirmed installable —
`pip` is network-blocked in this sandbox but the npm registry is reachable), `util.alignment`. Phase 3
Tier A and Phase 5 (`selector.py`) are both fully complete; Phase 4 (`util` Tier B) now has
`placement`/`geolocation`/`representation`/`cost` landed.
...
| `util.shape_builder` | 🔲 (part 1 in flight) | — | Split into two chunks given its unusual size (2312 lines, 60 methods) — Part 1 (vector-math free functions + `ShapeBuilder` class minus the four MEP transition/bend methods) dispatched to a background agent; Part 2 (`mep_transition_shape`/`mep_transition_length`/`mep_transition_calculate`/`mep_bend_shape`) to follow separately. Confirmed kernel-free (no `shapely` dependency) per the roadmap — only needs the matrix library already in place (`gl-matrix`). |
| `util.alignment` | 🔲 | — | — |
