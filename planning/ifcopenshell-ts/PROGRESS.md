...
| `api.aggregate` — `unassign_object` | ✅ | [#71](https://github.com/mikitski/IfcOpenShell/pull/71) | Ported alongside `api.spatial` above, as a direct dependency of `assign_container` — see that row. `assign_object` (the other half of this 2-function module, 160 LOC, its own "detach from old parent / cycle-adjacent invariant" complexity per `research/02-python-api-inventory.md` §3's "aggregate" deep dive) is a separate future chunk. |
