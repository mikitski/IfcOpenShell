...
//
// **THIS IS CHUNK 1 OF MANY** (matching `api.geometry`'s own 11+-PR precedent for a
// module of this size) -- only the 17 dependency-clean getter/query files below are
// landed here (~680 lines). None of the 17 is registered with `ifcopenshell.api.run`/
// an internal `Usecase` class (confirmed: real Python's own `__init__.py` imports
// every one of them as a plain function, no `Usecase`/`api.run` wiring anywhere in
// this module at all) -- all exported here as plain functions, matching real Python's
// own plain-function shape exactly.
//
// --- Landed in this chunk (17 files, ~680 lines) ---
//
...
// `distance_along_from_station`, `get_alignment_start_station`, `get_curve_segment`,
// `get_curve_segment_transition_code`, `get_layout_curve`, `get_mapped_segments`),
// their private `_`-prefixed helper files (`_add_segment_to_curve`,
