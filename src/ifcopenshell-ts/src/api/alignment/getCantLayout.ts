...
//
// Port of `ifcopenshell/api/alignment/get_cant_layout.py` (src/ifcopenshell-python, 31
// lines) -- structurally identical to `./getHorizontalLayout.ts`'s own port (same
...
/**
 * Returns the `IfcAlignmentCant` nested to this alignment (Python:
 * `ifcopenshell.api.alignment.get_cant_layout`).
 *
 * @param alignment The `IfcAlignment`.
 * @returns The `IfcAlignmentCant`, or `null` if none is nested to it.
 */
export function getCantLayout(alignment: EntityInstance): EntityInstance | null {
	for (const rel of alignment.get("IsNestedBy") as EntityInstance[]) {
		for (const layout of rel.get("RelatedObjects") as EntityInstance[]) {
			if (layout.isA("IfcAlignmentCant")) return layout;
		}
	}
	return null;
}
